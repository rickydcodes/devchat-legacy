...
router.get(
    ...
    async (req, res) => {
        res.render("main/rules", {
            title: "Rules",
            stylesheet: "main/rules",
            script: "",
...
router.get(
    ...
    middleware.bannedRedirect,
    async (req, res) => {
        let challenges = await server.db
...
router.get(
    ...
    async (req, res) => {
        ...
        if (["mostFollowers", "mostThreads", "mostPoints"].includes(sort)) {
